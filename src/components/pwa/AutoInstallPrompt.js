...
export const AutoInstallPrompt = () => {
    const { canPromptInstall, needsManualInstall, install, isInstalled, platform, isPreviewHost, isInIframe } = useInstallPrompt();
    const [visible, setVisible] = useState(false);
    const [busy, setBusy] = useState(false);
    const trackedShown = useRef(false);
    useEffect(() => {
        const checkVisibility = () => {
            if (isPreviewHost || isInIframe || isInstalled)
                return;
            ...
            try {
                ...
                if (dismissed && Date.now() - dismissed < DISMISS_MS)
                    return;
            }
            catch {
                /* ignore */
    ...
    if (!visible)
        return null;
    return (<div className="fixed bottom-4 left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-2rem)] max-w-md animate-in slide-in-from-bottom-6 fade-in duration-500">
      <div className="relative rounded-2xl border border-primary/30 bg-background/95 backdrop-blur-xl shadow-2xl shadow-primary/10 p-4">
        <button onClick={dismiss} aria-label="Fechar" className="absolute top-2 right-2 p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60">
          <X className="h-4 w-4"/>
        </button>
        ...
        <div className="mt-3 flex gap-2">
          {canPromptInstall ? (<Button onClick={handleInstall} size="sm" className="flex-1" disabled={busy}>
              <Download className="h-4 w-4"/>
              {busy ? "Instalando..." : "Instalar agora"}
            </Button>) : (<Button asChild size="sm" className="flex-1" onClick={dismiss}>
