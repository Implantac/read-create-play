...
import { runAllModels, getConsensusRanking } from "@/engine/ai/ml-models";
import { motion, AnimatePresence } from "framer-motion";
import { Brain, Play, Trophy, Target, Percent, Info, Beaker } from "lucide-react";
...
function BreakdownBar({ breakdown }) {
    ...
    return (<div className="flex gap-0.5 h-1.5 rounded-full overflow-hidden w-full">
      {factors.map(f => (<div key={f.key} className={`${f.color} opacity-80`} style={{ width: `${f.value}%` }} title={`${f.key}: ${f.value}%`}/>))}
    </div>);
}
function BacktestBadge({ model }) {
...
export function MLPanel({ stats, config, draws }) {
    ...
    const renderChart = (predictions, color) => {
        ...
        <ResponsiveContainer width="100%" height="100%">
          <BarChart data={top20} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
            ...
            <Bar dataKey="score" radius={[3, 3, 0, 0]} cursor="pointer" onClick={(data) => data && setSelectedNumber(data)}>
              {top20.map((_, i) => (<Cell key={i} fill={`hsl(${color}, ${80 - i * 2}%)`} fillOpacity={1 - i * 0.02}/>))}
            </Bar>
          </BarChart>
        </ResponsiveContainer>
      </div>);
    ...
    const renderBreakdownDetail = () => {
        ...
        return (<motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="rounded-lg bg-secondary/50 border border-border p-3 mb-3">
        ...
        <div className="space-y-1.5">
          {factors.map(f => (<div key={f.label} className="flex items-center gap-2">
    ...
    return (<div className="rounded-xl bg-card border border-border p-5">
      <div className="flex items-center justify-between mb-4">
        ...
        <Button size="sm" onClick={run} disabled={running} className="bg-neon-purple/20 text-neon-purple border border-neon-purple/30 hover:bg-neon-purple/30">
          <Play className="w-3 h-3 mr-1"/>
          {running ? "Processando..." : "Executar Modelos"}
        </Button>
      </div>

      {running && (<div className="flex flex-col items-center py-12 gap-3">
          <div className="w-8 h-8 border-2 border-neon-purple border-t-transparent rounded-full animate-spin"/>
          <p className="text-sm text-muted-foreground">Cálculo de evidência e calibração dos 6 modelos...</p>
        </div>)}

      {models && !running && (<AnimatePresence>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
            {/* Model stats cards */}
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
              {models.map((model) => (<div key={model.name} className="rounded-lg bg-secondary/50 border border-border p-3">
                  <p className="text-xs font-semibold text-foreground mb-2 truncate">{model.name}</p>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground mb-1">
                    <Target className="w-3 h-3 shrink-0"/>
                    Performance: <span className="text-foreground font-mono">{model.performanceScore !== null ? model.performanceScore.toFixed(1) : "N/A"}</span>
                  </div>
            ...
            <Tabs defaultValue="consensus" className="w-full">
              <TabsList className="w-full bg-secondary/50 border border-border flex-wrap h-auto gap-1 p-1">
                ...
                {models.map(m => (<TabsTrigger key={m.name} value={m.name} className="text-xs">
              ...
              <TabsContent value="consensus" className="mt-4">
                ...
                {consensus && (<>
                    ...
                    <div className="mt-3 space-y-1">
                      {consensus.slice(0, 5).map(p => (<div key={p.number} className="flex items-center gap-2">
              ...
              {models.map((model, mi) => {
                ...
                return (<TabsContent key={model.name} value={model.name} className="mt-4">
                    ...
                    <div className="mt-3 space-y-1">
                      {model.predictions.slice(0, 5).map(p => (<div key={p.number} className="flex items-center gap-2">
                          <span className="text-xs font-mono text-foreground w-8">Nº{String(p.number).padStart(2, '0')}</span>
                          <BreakdownBar breakdown={p.breakdown}/>
                          <span className="text-[10px] font-mono text-muted-foreground w-10 text-right">{p.score}%</span>
                        </div>))}
      ...
      {!models && !running && (<div className="text-center py-8 text-muted-foreground text-sm">
