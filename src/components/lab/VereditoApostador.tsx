...
export function VereditoApostador({ lift, zScore, pValue, grade, lotteryName }: VereditoApostadorProps) {
  ...
  let recommendation = "O sinal atual é estatisticamente indistinguível de ruído aleatório. Risco de perda total do capital alocado.";

  if (grade === 'E4' || (lift > 1.05 && pValue < 0.01)) {
    status = 'Apostar';
    colorClass = 'text-emerald-500 border-emerald-500/50 bg-emerald-500/10';
    Icon = ShieldCheck;
    recommendation = "Evidência robusta detectada. O sinal supera a variância aleatória com alta confiança. Recomendado seguir plano de banca padrão.";
  } else if (grade === 'E3' || (lift > 1.02 && pValue < 0.05)) {
    status = 'Reduzido';
    colorClass = 'text-blue-500 border-blue-500/50 bg-blue-500/10';
    Icon = ShieldCheck;
    recommendation = "Evidência forte, mas com margem de erro presente. Sugere-se exposição reduzida (50% do capital padrão).";
  } else if (isExperimental) {
    status = 'Observar';
    colorClass = 'text-amber-500 border-amber-500/50 bg-amber-500/10';
    Icon = AlertTriangle;
    recommendation = "Sinal exploratório detectado, mas sem significância estatística rigorosa (P > 0.05). Recomendado apenas para observação ou testes em simulador.";
  }
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
          ...
          <p className="text-[10px] text-amber-500/80 italic leading-tight">
            Atenção: A análise quantitativa identifica anomalias estatísticas históricas. Loterias são eventos aleatórios por definição oficial e não há garantia de lucros futuros.
          </p>
        </div>
      </CardContent>
    </Card>
