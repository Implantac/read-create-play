function normalizeAndRank(scored) {
...
// ═══════════════════════════════════════════════════════
function backtestModel(modelFn, sortedDraws, // Must be sorted ASC
config, computeStatsFn, windowSize = 100, testSize = 50) {
    ...
    for (let i = 0; i < maxTestable; i++) {
        ...
        if (!testDraw || trainingDraws.length < 50)
            continue;
        ...
        let hits = 0;
        let t5hits = 0;
        for (const n of top15Numbers) {
            if (drawSet.has(n))
                hits++;
        }
        for (const n of top5Numbers) {
            if (drawSet.has(n))
                t5hits++;
        }
...
function computeAccuracyFromBacktest(bt, config) {
...
// ═══════════════════════════════════════════════════════
function buildBreakdown(freq, recency, trend, cycle, momentum, consistency, other) {
...
export function runFrequencyTrendScore(stats, config, excludedFeatures = []) {
...
export function runMultiFactorScore(stats, config) {
    const scored = stats.map(s => {
...
export function runTemporalPatternScore(stats, config) {
    const scored = stats.map(s => {
...
export function runBayesianScore(stats, config) {
...
export function runTransitionScore(stats, config) {
    const scored = stats.map(s => {
...
export function runMultiDimensionalPatternScore(stats, config) {
    const scored = stats.map(s => {
...
// ═══════════════════════════════════════════════════════
export function runEnsembleVoting(stats, config, modelWeights) {
    ...
    models.forEach(({ result, name }) => {
        ...
        result.predictions.forEach(p => {
            ...
            if (top30.has(p.number))
                numberScores[p.number].agreement++;
            if (p.breakdown) {
    ...
    const scored = Object.entries(numberScores).map(([num, data]) => {
...
// ═══════════════════════════════════════════════════════
export function runAllModels(stats, config, draws, computeStatsFn) {
    const modelFns = [
    ...
    if (draws && draws.length > 100 && computeStatsFn) {
        const ensembleFn = (s, c) => runEnsembleVoting(s, c, totalWeight > 0 ? calibratedWeights : undefined);
        const sortedDraws = [...draws].sort((a, b) => a.concurso - b.concurso);
...
export function getConsensusRanking(models) {
    const numberScores = {};
    ...
    models.forEach(model => {
        ...
        model.predictions.forEach(p => {
            ...
            if (top15.has(p.number))
                numberScores[p.number].topCount += 1;
            if (p.breakdown) {
 ...
 */
function buildReason(p, totalModels) {
    const bd = p.breakdown;
    if (!bd)
        return "Recomendação baseada em convergência algorítmica de alta precisão.";
    const factors = [
