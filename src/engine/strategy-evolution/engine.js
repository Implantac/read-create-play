...
import { generateByStrategy } from "@/engine/strategies";
...
// BACKTESTING
// ═══════════════════════════════════════════════════════
function backtestStrategy(strategyDef, games, draws, config) {
    ...
    const globalScore = Math.min(100, avgHits * 8 +
        consistency * 20 +
        diversityScore * 0.15 +
        coverageScore * 0.1 +
        (totalPrizes > 0 ? 15 : 0) -
...
function getCostForLottery(lotteryId) {
...
// RANKING
// ═══════════════════════════════════════════════════════
function buildRanking(results, lotteryId) {
...
function buildExplanation(def, m, rank) {
    const parts = [];
    if (rank === 1) {
    ...
    if (m.consistency > 0.7)
        parts.push("com alta consistência");
    else if (m.consistency < 0.3)
        parts.push("com baixa consistência");
    if (m.avgHits > 0)
        parts.push(`média de ${m.avgHits.toFixed(2)} acertos`);
    if (m.totalPrizes > 0)
        parts.push(`${m.totalPrizes} premiações detectadas`);
    if (m.diversityScore > 70)
        parts.push("boa diversidade entre jogos");
    if (m.lift > 1.05)
        parts.push(`evidência estatística positiva (${m.lift.toFixed(2)}x)`);
    return parts.join(", ") + ".";
}
// ═══════════════════════════════════════════════════════
// AUTO-EVOLUTION
// ═══════════════════════════════════════════════════════
function generateEvolutionSuggestions(rankings, results) {
    const suggestions = [];
...
// INSIGHTS
// ═══════════════════════════════════════════════════════
function generateInsights(rankings, lotteryId) {
    const insights = [];
    if (rankings.length === 0)
        return ["Nenhuma estratégia testada ainda."];
...
// MAIN LAB RUNNER
// ═══════════════════════════════════════════════════════
export function runStrategyLab(labConfig, allDraws, config) {
    const start = performance.now();
    // Filter draws to range
    const draws = allDraws.filter(d => d.concurso >= labConfig.drawRange[0] && d.concurso <= labConfig.drawRange[1]);
    if (draws.length === 0) {
        return {
            config: labConfig,
            rankings: [],
    ...
    const selectedDefs = labConfig.strategies.length > 0
        ? labConfig.strategies.map(id => getStrategy(id)).filter(Boolean)
        : available;
    ...
    for (const def of selectedDefs) {
        ...
        for (let i = 0; i < labConfig.gamesPerStrategy * 3 && games.length < labConfig.gamesPerStrategy; i++) {
            const game = generateByStrategy(def.baseStrategy, trainStats, config);
    ...
    const generatedGames = rankings.map(r => {
