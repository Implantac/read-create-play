...
import { Users, Crown, TrendingUp, Search, Shield, Loader2, RefreshCw, ShieldCheck, AlertTriangle, History, UserCog, Grid3X3, DollarSign, ArrowUpRight, Target, FlaskConical, Smartphone } from "lucide-react";
...
export default function AdminPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        if (drawsRes.count !== null)
            setDrawCount(drawsRes.count);
        if (rolesRes.data)
    ...
    const executeRoleUpdate = async (userId, newRole, oldRole) => {
        setUpdating(userId);
        ...
        try {
            ...
            if (newRole === "user") {
                ...
                setRoles(prev => [...prev, { id: crypto.randomUUID(), user_id: userId, role: newRole }]);
            }
    ...
    return (<div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground flex items-center gap-2">
            Painel Admin
            {isSuperAdmin && (<Badge className="bg-amber-500/20 text-amber-400 border border-amber-500/30 text-[10px] uppercase">
                Super Admin
              </Badge>)}
          </h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map(s => (<Card key={s.label} className="bg-card/60 border-border/50">
      ...
      <Tabs defaultValue="users" className="space-y-4">
        ...
        <TabsContent value="users">
          ...
          <Card className="bg-card/60 border-border/50 mb-4">
            ...
            if (pct === 0)
                return null;
            return (<div key={plan} className={`h-full flex items-center justify-center text-[10px] font-bold transition-all ${plan === "free" ? "bg-muted-foreground/30 text-foreground" :
                    plan === "premium" ? "bg-primary text-primary-foreground" :
                        plan === "lifetime" ? "bg-yellow-500 text-yellow-950" :
                            "bg-accent text-accent-foreground"}`} style={{ width: `${pct}%` }}>
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="relative w-52">
                    ...
                    <Input placeholder="Buscar..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-10 h-9"/>
                  </div>
            ...
            <CardContent>
              {loading ? (<div className="flex justify-center py-8">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filtered.map(p => (<UserRow key={p.id} profile={p} role={getUserRole(p.id)} isUpdating={updating === p.id} onUpdatePlan={updatePlan} onUpdateRole={updateRole} onToggleBlock={toggleBlock} onViewDetails={setDetailUser} planLabels={PLAN_LABELS} planColors={PLAN_COLORS} roleLabels={ROLE_LABELS} roleColors={ROLE_COLORS}/>))}
        ...
        ].map(item => (<Card key={item.role} className="bg-card/40 border-border/30">
        ...
        ].map(item => (<div key={item.name} className="flex items-center justify-between p-3 rounded-xl bg-white/5 border border-white/5">
                    <div>
        ...
        <TabsContent value="audit">
          ...
          <Card className="bg-card/60 border-border/50">
            ...
            <CardContent>
              {auditLogs.length === 0 ? (<p className="text-center text-muted-foreground py-8">Nenhum registro de auditoria encontrado.</p>) : (<AuditLogTable logs={auditLogs} getAdminEmail={getAdminEmail} getTargetEmail={getTargetEmail}/>)}
            </CardContent>
          </Card>
        </TabsContent>
      ...
      <Dialog open={!!detailUser} onOpenChange={(o) => !o && setDetailUser(null)}>
        <DialogContent>
          ...
          {detailUser && (<div className="space-y-3">
              <div className="grid grid-cols-2 gap-3 text-sm">
                ...
                <div>
                  ...
                  <Badge variant={!isFullAccessEmail(detailUser.email) && detailUser.blocked ? "destructive" : "outline"} className={!isFullAccessEmail(detailUser.email) && detailUser.blocked ? "" : "text-green-500 border-green-500/30"}>
